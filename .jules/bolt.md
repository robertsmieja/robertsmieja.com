...
## 2026-04-20 - Optimized Blog Collection Sorting
**Learning:** Repeated `Date` instantiation within sort comparators (e.g., `(a, b) => new Date(b.date) - new Date(a.date)`) for blog collections is a performance bottleneck, scaling as $O(N \log N)$.
**Action:** Use a map-sort-map pattern (Schwartzian transform) to cache parsed date values before sorting, reducing date parsing overhead to $O(N)$.
