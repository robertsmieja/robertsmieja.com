...
## 2025-05-18 - Optimized Array Date Sorting using Schwartzian Transform

**Learning:** When sorting a large collection of objects by a date string (e.g., Astro collection entries), creating a `new Date()` object inside the array `.sort()` comparator causes the date to be parsed O(N log N) times. In a static site generator this needlessly slows down the build time, and in a SSR context it creates a hot path. Furthermore, using the object spread operator (e.g. `{ ...post }`) when transforming Astro collections strips the object of its prototype methods (like `.render()`), causing downstream crashes.
**Action:** Extract sorting logic into a shared utility file. Apply the Schwartzian transform (map-sort-map) to parse the date strings exactly once per item, reducing time complexity for date parsing to O(N). To preserve prototype methods during the transform, wrap the original object rather than spreading it (e.g. `{ post, dateValue: new Date(post.data.date).valueOf() }`), then unwrap it after sorting.
