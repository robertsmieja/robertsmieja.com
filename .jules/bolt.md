...
## 2025-04-28 - Optimized Blog Date Sorting

**Learning:** Repeated `Date` instantiation within sort comparators for blog collections is a performance bottleneck. Using a map-sort-map pattern (Schwartzian transform) to cache parsed date values reduces date parsing from O(N log N) to O(N), improving sort performance.

**Action:** Extracted blog sorting logic into a reusable `sortPostsByDate` utility function in `src/lib/blogUtils.ts` that implements the Schwartzian transform.
